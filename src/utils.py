...
def generate_persona_export(
    ...
    lines.append("---")
    lines.append("")
    lines.append("## Market Scenario Context (Stress-Test References)")
    lines.append("")
    lines.append("Use these two historical scenarios as stress-test anchors — not as predictions.")
    lines.append("")
    lines.append("### Scenario A: COVID Crash + Recovery (2020)")
    lines.append("- Trigger: Exogenous demand shock (global pandemic)")
    lines.append("- S&P 500 drawdown: −34% in ~33 days (fastest crash on record)")
    lines.append("- Recovery: Full S&P recovery in ~5 months; NASDAQ ended 2020 up +30.7%")
    lines.append("- Bond behavior: 10Y Treasury yield collapsed to 0.5% ATL (flight to safety)")
    lines.append("- Fed response: $1.5T+ liquidity injection within days")
    lines.append("- Key lesson: Speed of central bank intervention determines recovery shape.")
    lines.append("  Growth/tech assets recover fastest in liquidity-driven rebounds.")
    lines.append("- IRP implication: Short, sharp crashes favor holding and rebalancing at trough.")
    lines.append("")
    lines.append("### Scenario B: IT Bubble Crash + Recovery (2000–2002)")
    lines.append("- Trigger: Valuation collapse (NASDAQ P/E >200), rate hikes, accounting scandals, 9/11")
    lines.append("- NASDAQ drawdown: −78% over ~31 months; took 15 years to recover (2015)")
    lines.append("- S&P 500 drawdown: −49%; recovery took ~7 years (2007)")
    lines.append("- Bond behavior: 10Y Treasury rallied strongly (bonds and equities diverged)")
    lines.append("- Fed response: Cut rates 6.5% → 1% but recovery was still slow")
    lines.append("- Key lesson: Valuation-driven crashes have no natural floor and recover on")
    lines.append("  fundamentals, not liquidity. A 2030 target portfolio must account for 7+ year dead zones.")
    lines.append("- IRP implication: If tech-heavy ETFs reprice sharply, >5% drift rebalancing may")
    lines.append("  trigger repeated buys into a falling knife. Allocation targets need review.")
    lines.append("")
    ...
    lines.append("**Each persona MUST reference Scenario A and/or Scenario B above** to stress-test their recommendations.")
    lines.append("")
    ...
    lines.append("- **Scenario anchor:** Use COVID recovery (Scenario A) as your base case.")
    lines.append("  Argue that innovation assets decouple from macro shocks and lead rebounds.")
    lines.append("  Challenge IT Bubble comparisons by pointing to real revenue and adoption vs. vaporware.")
    lines.append("")
    ...
    lines.append("- **Scenario anchor:** Use fundamentals as the filter across BOTH scenarios.")
    ...
    lines.append("- **Scenario anchor:** Treat both scenarios as debt/credit cycle events.")
    lines.append("  COVID = fastest-ever policy intervention; reward came from holding through the print.")
    lines.append("  IT Bubble = classic speculative excess correction with no policy shortcut.")
    lines.append("  Emphasize that bond behavior diverged completely between the two — this is the key")
    lines.append("  signal for which regime the current environment resembles.")
    lines.append("")
    ...
    lines.append("   - `### Discussion` — summarize the key debates, how each persona influenced the final allocation, and what the scenario analysis implies for the IRP portfolio's allocation targets")
    ...
    lines.append("   - `### Market Outlook` — macro view. MUST answer: Which scenario (A: COVID or B: IT Bubble) does the current market most resemble, and why?")
    lines.append("   - `### Implementation Plan` — phased execution steps. MUST answer: What specific rebalancing action (if any) is recommended before the next snapshot date?")
    lines.append("")
